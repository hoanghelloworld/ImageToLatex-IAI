...
# Hàm train để huấn luyện mô hình
def train(args):
   ...
   # Kiểm tra bộ nhớ GPU nếu sử dụng GPU
   ...
   # Nếu có checkpoint, tải trọng số mô hình từ checkpoint
       ...
       for e in range(args.epoch, args.epochs):
           ...
           for i, (seq, im) in enumerate(dset):
               if seq is not None and im is not None:
                   opt.zero_grad()
                   total_loss = 0
                   # Chia dữ liệu thành các micro batch
...
# Khởi tạo phân tích đối số dòng lệnh
