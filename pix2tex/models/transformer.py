...
class CustomARWrapper(AutoregressiveWrapper):
    ...
    def generate(self, start_tokens, seq_len=256, eos_token=None, temperature=1., filter_logits_fn=top_k, filter_thres=0.9, **kwargs):
        ...
        # Nếu start_tokens là tensor 1 chiều thì thêm chiều batch size là 1
        if num_dims == 1:
        ...
        for _ in range(seq_len):
            ...
            # Áp dụng hàm lọc logits (top_k hoặc top_p) và tính xác suất
            ...
            # Nếu gặp eos_token thì dừng
            if eos_token is not None and (torch.cumsum(out == eos_token, 1)[:, -1] >= 1).all():
                break

        out = out[:, t:]

        # Nếu ban đầu start_tokens là tensor 1 chiều thì squeeze chiều batch size
        if num_dims == 1:
            out = out.squeeze(0)
