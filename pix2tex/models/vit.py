...
class ViTransformerWrapper(nn.Module):
    def __init__(
        ...
        *,
        ...
        # Khởi tạo embedding cho vị trí của các patch
        self.pos_embedding = nn.Parameter(torch.randn(1, num_patches + 1, dim))
        # Tạo linear layer để chuyển đổi patch thành embedding
        self.patch_to_embedding = nn.Linear(patch_dim, dim)
        # Khởi tạo cls token 
        self.cls_token = nn.Parameter(torch.randn(1, 1, dim))
        # Tạo dropout layer cho embedding
        self.dropout = nn.Dropout(emb_dropout)

        # Các tầng attention
        self.attn_layers = attn_layers
        # Layer norm sau khi qua attention
    ...
    def forward(self, img, **kwargs):
        ...
        # Chia ảnh thành các patch và chuyển về dạng embedding
        x = rearrange(img, 'b c (h p1) (w p2) -> b (h w) (p1 p2 c)', p1=p, p2=p)
        x = self.patch_to_embedding(x)
        b, n, _ = x.shape

        # Thêm cls token vào đầu
        cls_tokens = repeat(self.cls_token, '() n d -> b n d', b=b)
        x = torch.cat((cls_tokens, x), dim=1)
        # Tính toán vị trí embedding
